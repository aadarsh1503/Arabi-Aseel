...
import { Plus, Edit, Trash2, XCircle, UploadCloud, User, Star, ArrowLeft, LogOut, CheckCircle, AlertTriangle, Info, ShieldCheck } from 'lucide-react'; // ShieldCheck added
...
const Chef = () => {
    ...
    // NEW State for the confirmation flow
    ...
    // STEP 1 of Submission: Validate and open confirmation modal
    ...
    // STEP 2 of Submission: The actual API call after confirmation
        ...
        <div dir={isRTL ? 'ltr' : 'ltr'} className={`min-h-screen font-noto-serif bg-slate-50 font-sans text-slate-800`}>
            ...
            <div className="container font-noto-serif mx-auto p-4 md:p-8">
                <header className="flex justify-between items-center mb-12">
                    <button onClick={() => navigate('/admin')} className="flex items-center space-x-2 px-4 py-2 rounded-lg text-slate-600 hover:bg-slate-200 transition-colors" title="Back to Admin Panel"><ArrowLeft size={20} /><span className="font-semibold hidden md:inline">{t('back', 'Back')}</span></button>
                    <h1 className="text-4xl md:text-5xl font-noto-serif font-bold bg-gradient-to-r from-cyan-500 to-blue-600 bg-clip-text text-transparent pb-2 text-center">{t('chefsPageTitle', 'Manage Culinary Artists')}</h1>
